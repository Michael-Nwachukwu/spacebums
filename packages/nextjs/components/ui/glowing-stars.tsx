...
export const GlowingStarsBackgroundCard = ({
  ...
  return (
    <div
      ...
      className={cn(
        "bg-[linear-gradient(110deg,#333_0.6%,#222)] p-4 max- w-96 max-h[20rem] h-full w-ful rounded-xl border border-slate-700 dark:border-neutral-600",
        className,
      )}
    >
      ...
      <div className="pt-6 pb-2 space-y-2">{children}</div>
    </div>
  );
};
...
export const GlowingStarsDescription = ({
  ...
  return <p className={cn("text-base max-w[16rem] font-light text-gray-300", className)}>{children}</p>;
};
...
export const GlowingStarsTitle = ({ className, children }: { className?: string; children?: React.ReactNode }) => {
  return (
    <h2
      className={cn(
        "font-bold text-2xl bg-gradient-to-b from-neutral-200 to-neutral-500 bg-clip-text text-transparent",
        className,
      )}
    >
      {children}
